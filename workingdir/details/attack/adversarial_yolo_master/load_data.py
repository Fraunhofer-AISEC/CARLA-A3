...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        #confs_for_class = normal_confs[:, 11, :] # 11 is for stopsign
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        pad = (img_size - adv_patch.size(-1)) / 2 # paste on the middle of the image
...
class PatchStopTransformer(nn.Module):
    """PatchTransformer: transforms batch of patches
    ...
    def __init__(self):
        super(PatchStopTransformer, self).__init__()
        self.min_contrast = 0.8
        self.max_contrast = 1.2
        self.min_brightness = -0.1
        self.max_brightness = 0.1
        ...
        textMask = np.loadtxt(os.path.join(os.path.dirname(os.path.abspath(__file__)), "masks/base_mask.txt")) # load the mask of the stop sign
        textMask = torch.from_numpy(textMask).float().cuda()
        self.textmsk_batch = textMask.unsqueeze(0).unsqueeze(0)
        ...
        self.kernel = kernel.unsqueeze(0).unsqueeze(0).expand(3,3,-1,-1)
        '''
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=False, rand_loc=False):
        #adv_patch = F.conv2d(adv_patch.unsqueeze(0),self.kernel,padding=(2,2))
        txt_batch = F.interpolate(self.textmsk_batch, size=(img_size),mode = 'nearest')
        txt_batch = txt_batch.repeat(1,3,1,1)
        #self.txt_batch = self.txt_batch.unsqueeze(0)
        txt_batch = txt_batch.unsqueeze(0)
        txt_batch = txt_batch.expand(lab_batch.size(0), lab_batch.size(1),-1,-1,-1)
        ...
        lab_batch_scaled[:, :, 3] = lab_batch[:, :, 3] * img_size
        lab_batch_scaled[:, :, 4] = lab_batch[:, :, 4] * img_size
        target_size = lab_batch_scaled[:, :, 3] # set to cover the whole stop sign
        target_x = lab_batch[:, :, 1].view(np.prod(batch_size))
        target_y = lab_batch[:, :, 2].view(np.prod(batch_size))
        targetoff_x = lab_batch[:, :, 3].view(np.prod(batch_size))
        targetoff_y = lab_batch[:, :, 4].view(np.prod(batch_size))
        ...
        adv_batch = adv_batch.view(s[0] * s[1], s[2], s[3], s[4])
        msk_batch = msk_batch.view(s[0] * s[1], s[2], s[3], s[4])
        txt_batch = txt_batch.view(s[0] * s[1], s[2], s[3], s[4])
        ...
        adv_batch_t = F.grid_sample(adv_batch, grid)
        msk_batch_t = F.grid_sample(txt_batch, grid)
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch):
        ...
        for adv in advs:
            img_batch = torch.where((adv == 0), img_batch, adv) # paste
        return img_batch
